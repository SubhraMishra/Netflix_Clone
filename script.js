...
const apikey="918d66fe1c9322634e4150896f799add";
...
function buildBannerSection(movie){
    ...
    <p class="banner_info">Trending in Movies |Average rating- ${movie.vote_average}</p>
...
function buildMoviesSection(list,categoryName){
    ...
    <h2 class="movie-section-heading">${categoryName}<span class="explore-nudge">explore</span></h2> 
...
window.addEventListener('load',function(){
    ...
    window.addEventListener('scroll',function(){
        ...
        if(window.scrollY>100) header.classList.add('bg_black')
        else header.classList.remove('bg_black');
    })

    window.addEventListener('click',function(){
      const toggle1=document.getElementById('explore')
    })
})
